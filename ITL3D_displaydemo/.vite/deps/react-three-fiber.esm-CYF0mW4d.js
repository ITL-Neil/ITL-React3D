...
import { Camera, Clock, Color, Layers, LinearEncoding, OrthographicCamera, PerspectiveCamera, Raycaster, Scene, Texture, Vector2, Vector3, WebGLRenderer, sRGBEncoding, t as three_module_exports } from "./three.js";
...
var useIsomorphicLayoutEffect$1 = typeof window === "undefined" || !window.navigator || /ServerSideRendering|^Deno\//.test(window.navigator.userAgent) ? import_react.useEffect : import_react.useLayoutEffect;
...
//#endregion
//#region node_modules/suspend-react/dist/index.js
...
function query(fn, keys, preload = false, config = {}) {
	for (const entry of globalCache) if (shallowEqualArrays(keys, entry.keys, entry.equal)) {
		...
		if (Object.prototype.hasOwnProperty.call(entry, "response")) return entry.response;
	...
	const entry = {
		...
		promise: fn(...keys).then((response) => entry.response = response).then(() => {
			if (config.lifespan && config.lifespan > 0) setTimeout(() => {
...
//#endregion
//#region node_modules/@react-three/fiber/dist/index-af244448.esm.js
...
function prepare(object, state) {
	...
	if (state != null && state.primitive || !instance.__r3f) instance.__r3f = {
		type: "",
		root: null,
		previousAttach: null,
		memoizedProps: {},
...
function applyProps$1(instance, data) {
	...
	changes.forEach(([key, value, isEvent, keys]) => {
		...
		if (value === DEFAULT + "remove") if (targetProp && targetProp.constructor) {
			var _memoized$args;
		...
		if (isEvent) {
			...
			if (Array.isArray(value)) if (targetProp.fromArray) targetProp.fromArray(value);
			...
			else if (targetProp.copy && value && value.constructor && targetProp.constructor.name === value.constructor.name) targetProp.copy(value);
				...
				if (!("ColorManagement" in three_module_exports) && !rootState.linear && isColor) targetProp.convertSRGBToLinear();
			}
		} else {
			currentInstance[key] = value;
			if (!rootState.linear && currentInstance[key] instanceof Texture) currentInstance[key].encoding = sRGBEncoding;
		}
...
function getEventPriority() {
	...
	switch ((_window = window) == null ? void 0 : (_window$event = _window.event) == null ? void 0 : _window$event.type) {
		case "click":
		case "contextmenu":
		case "dblclick":
		case "pointercancel":
...
function createEvents(store) {
	...
	function intersect(event, filter) {
		...
		if ("pointerId" in event && state.internal.capturedMap.has(event.pointerId)) for (let captureData of state.internal.capturedMap.get(event.pointerId).values()) intersections.push(captureData.intersection);
...
function createRenderer(_roots, _getEventPriority) {
	function createInstance(type, { args = [], attach, ...props }, root) {
		...
		if (type === "primitive") {
			if (props.object === void 0) throw `Primitives without 'object' are invalid!`;
			...
			if (!target) throw `${name} is not part of the THREE namespace! Did you forget to extend? See: https://docs.pmnd.rs/react-three-fiber/api/objects#using-3rd-party-objects-declaratively`;
	...
	function removeChild(parentInstance, child, dispose) {
		if (child) {
			...
			if ((_child$__r3f3 = child.__r3f) != null && _child$__r3f3.attach) detach(parentInstance, child, child.__r3f.attach);
				...
				if ((_child$__r3f4 = child.__r3f) != null && _child$__r3f4.root) removeInteractivity(child.__r3f.root, child);
			}
			const isPrimitive = (_child$__r3f5 = child.__r3f) == null ? void 0 : _child$__r3f5.primitive;
			const shouldDispose = dispose === void 0 ? child.dispose !== null && !isPrimitive : dispose;
			if (!isPrimitive) {
				var _child$__r3f6;
			...
			if (shouldDispose && child.dispose && child.type !== "Scene") (0, import_scheduler.unstable_scheduleCallback)(import_scheduler.unstable_IdlePriority, () => {
				try {
					child.dispose();
				} catch (e) {}
			});
	...
	function switchInstance(instance, type, newProps, fiber) {
		...
		if (newInstance.raycast && newInstance.__r3f.eventCount) newInstance.__r3f.root.getState().internal.interaction.push(newInstance);
...
function createRoot(canvas) {
	...
	return {
		configure(props = {}) {
			let { gl: glConfig, size, events, onCreated: onCreatedCallback, shadows = false, linear = false, flat = false, legacy = false, orthographic = false, frameloop = "always", dpr = [1, 2], performance, raycaster: raycastOptions, camera: cameraOptions, onPointerMissed } = props;
			...
			if (gl.shadowMap) {
				...
				if (isBoolean && gl.shadowMap.enabled !== shadows || !is.equ(shadows, gl.shadowMap, shallowLoose)) {
					...
					else gl.shadowMap.type = 2;
					if (old !== gl.shadowMap.enabled) gl.shadowMap.needsUpdate = true;
				}
			}
			if ("ColorManagement" in three_module_exports) setDeep(three_module_exports, legacy, ["ColorManagement", "legacyMode"]);
			const outputEncoding = linear ? LinearEncoding : sRGBEncoding;
			const toneMapping = flat ? 0 : 4;
		...
		render(children) {
			...
			reconciler.updateContainer(/* @__PURE__ */ import_react.createElement(Provider, {
				store,
				children,
				onCreated,
				rootElement: canvas
...
function Provider({ store, children, onCreated, rootElement }) {
	...
	return /* @__PURE__ */ import_react.createElement(context.Provider, { value: store }, children);
}
	...
	*    {createPortal(...)} */
	...
	const inject = import_react.useCallback((rootState, injectState) => {
		...
		Object.keys(rootState).forEach((key) => {
			if (privateKeys.includes(key) || rootState[key] !== injectState[key]) delete intersect[key];
		});
		let viewport = void 0;
		if (injectState && size) {
	...
	return /* @__PURE__ */ import_react.createElement(import_react.Fragment, null, reconciler.createPortal(/* @__PURE__ */ import_react.createElement(context.Provider, { value: usePortalStore }, children), usePortalStore, null));
}
...
*/
var Canvas = /* @__PURE__ */ import_react.forwardRef(function Canvas({ children, fallback, resize, style, gl, events = createPointerEvents, shadows, linear, flat, legacy, orthographic, frameloop, dpr, performance, raycaster, camera, onPointerMissed, onCreated, ...props }, forwardedRef) {
	...
	if (containerRect.width > 0 && containerRect.height > 0 && canvas) {
		...
		root.current.configure({
			...
			onCreated: (state) => {
				state.events.connect == null || state.events.connect(divRef.current);
				onCreated?.(state);
			}
		});
		root.current.render(/* @__PURE__ */ import_react.createElement(ErrorBoundary, { set: setError }, /* @__PURE__ */ import_react.createElement(import_react.Suspense, { fallback: /* @__PURE__ */ import_react.createElement(Block, { set: setBlock }) }, children)));
	}
	useIsomorphicLayoutEffect(() => {
	...
	}, props), /* @__PURE__ */ import_react.createElement("div", {
		ref: containerRef,
		style: {
			width: "100%",
			height: "100%"
		}
	}, /* @__PURE__ */ import_react.createElement("canvas", {
		ref: canvasRef,
		style: { display: "block" }
	}, fallback)));
});
//#endregion
export { useGraph as C, suspend as D, useThree as E, create as O, useFrame as S, useStore as T, reconciler as _, addEffect as a, threeTypes as b, applyProps as c, createPortal as d, createRoot as f, invalidate as g, getRootState as h, addAfterEffect as i, _extends as k, context as l, extend as m, createPointerEvents as n, addTail as o, dispose as p, act as r, advance as s, Canvas as t, createEvents as u, render as v, useLoader as w, unmountComponentAtNode as x, roots as y };

//# sourceMappingURL=react-three-fiber.esm-CYF0mW4d.js.map